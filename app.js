...
const i18n = {
    en: {
        ...
        behavioral:'Behavioral', technical:'Knowledge Q&A', caseStudy:'Case Study',
        ...
        technicalTitle:'Knowledge Q&A',
        technicalDesc:'These are the kinds of questions that may come up to check your basic understanding. Perfect textbook answers are not expected. Explain in your own words and show genuine interest in learning.',
    ...
    ja: {
        ...
        behavioral:'行動面接', technical:'知識Q&A', caseStudy:'ケーススタディ',
        ...
        technicalTitle:'知識Q&A',
        technicalDesc:'基本的な理解度を確認するために聞かれる可能性がある質問です。教科書的な完璧な回答は不要。自分の言葉で説明し、学ぶ意欲を見せることが大切です。',
        caseTitle:'ケーススタディ',
...
const technicalQuestions = [
    { id:'tq1', question:'What does the Operations department do at an investment bank?', questionJa:'投資銀行のオペレーション部門は何をする部署ですか？',
      answer:'Operations (Ops) is the backbone that makes trading actually work. After a trader agrees to buy or sell, Ops handles everything that follows:\n\n\u2022 Confirming trade details with the counterparty\n\u2022 Making sure securities and cash are exchanged correctly (settlement)\n\u2022 Checking that all records match (reconciliation)\n\u2022 Processing corporate events like dividends\n\u2022 Managing risk and ensuring regulatory compliance\n\nThink of it this way: the front office (traders) decides WHAT to buy/sell. Operations makes sure the deal actually HAPPENS correctly and on time. Without Ops, trades would not settle, records would not match, and the bank could face huge financial and legal problems.',
      answerJa:'オペレーション（Ops）は取引を実際に成立させる屋台骨です。トレーダーが売買を決めた後、その先の全てをOpsが担います：\n\n\u2022 カウンターパーティと取引内容を確認\n\u2022 証券と現金が正しく交換されることを確認（決済）\n\u2022 全ての記録が一致しているかチェック（照合）\n\u2022 配当などのコーポレートイベントの処理\n\u2022 リスク管理と規制対応\n\nこう考えるとわかりやすいです：フロントオフィス（トレーダー）は「何を売買するか」を決める。オペレーションは「その取引が正しく、期限通りに完了する」ことを確実にする。Opsがなければ取引は決済されず、記録は合わず、銀行は巨額の損失や法的問題に直面します。',
      beginnerNote:'Ops is like the logistics department of a trading floor. Traders make deals, Ops makes sure the deals are completed correctly.',
      beginnerNoteJa:'Opsはトレーディングフロアの物流部門のようなもの。トレーダーが取引を決め、Opsがその取引が正しく完了するようにします。' },
    { id:'tq2', question:'Can you walk me through the trade lifecycle?', questionJa:'トレードライフサイクル（取引の流れ）を説明してください。',
      answer:'The trade lifecycle is the journey a trade takes from start to finish:\n\n1. Execution: A trader agrees to buy or sell with a counterparty (on an exchange or over-the-counter)\n2. Booking: The trade is recorded in the bank\'s internal systems\n3. Confirmation: Both sides confirm they agree on the details (price, quantity, settlement date)\n4. Clearing: For exchange trades, a clearinghouse (CCP) steps in between buyer and seller to reduce risk\n5. Settlement: Securities and cash are actually exchanged (in the US, this happens T+1, one business day after the trade)\n6. Reconciliation: Ops checks that all records match across systems\n\nOperations is involved from step 2 onward. The goal is for most of this to happen automatically (called STP, Straight-Through Processing), but when something goes wrong, Ops investigates and fixes it.',
      answerJa:'トレードライフサイクルは、取引が始まってから完了するまでの流れです：\n\n1. 約定（Execution）：トレーダーがカウンターパーティと売買を合意（取引所またはOTC）\n2. 記帳（Booking）：取引を銀行の社内システムに記録\n3. 確認（Confirmation）：両者が取引内容（価格、数量、決済日）に合意していることを確認\n4. 清算（Clearing）：取引所取引の場合、清算機関（CCP）が買い手と売り手の間に入りリスクを軽減\n5. 決済（Settlement）：証券と現金が実際に交換される（米国ではT+1、取引の翌営業日）\n6. 照合（Reconciliation）：Opsがシステム間で全記録が一致しているか確認\n\nオペレーションはステップ2以降に関わります。ほとんどを自動処理（STP）することが目標ですが、問題が起きた時はOpsが調査・修正します。',
      beginnerNote:'Think of buying something online: you click buy (execution), the order is logged (booking), you get a confirmation email (confirmation), payment is processed (clearing), and the item is delivered (settlement).',
      beginnerNoteJa:'ネット通販を想像してください：購入ボタンを押す（約定）、注文が記録される（記帳）、確認メールが届く（確認）、支払い処理（清算）、商品が届く（決済）。' },
    { id:'tq3', question:'What is the difference between stocks and bonds?', questionJa:'株式と債券の違いは何ですか？',
      answer:'Stocks (Equities):\n\u2022 You own a piece of the company\n\u2022 Value goes up or down with the company\'s performance and market sentiment\n\u2022 You may receive dividends (a share of profits), but they are not guaranteed\n\u2022 Higher risk, higher potential return\n\nBonds (Fixed Income):\n\u2022 You are lending money to a company or government\n\u2022 You receive regular interest payments (coupons) and get your money back at maturity\n\u2022 Generally lower risk than stocks (especially government bonds)\n\u2022 Lower potential return\n\nWhy this matters for Ops: Stocks and bonds settle differently, have different corporate actions, and involve different systems. Ops teams are often organized by asset class (equities Ops, fixed income Ops, etc.).',
      answerJa:'株式（エクイティ）：\n\u2022 会社の一部を所有する\n\u2022 会社の業績や市場の雰囲気で価値が上下する\n\u2022 配当（利益の分配）を受け取れるが、保証はない\n\u2022 リスクが高い分、リターンも高い可能性\n\n債券（フィックストインカム）：\n\u2022 会社や政府にお金を貸している\n\u2022 定期的に利息（クーポン）を受け取り、満期に元本が返ってくる\n\u2022 一般的に株式より低リスク（特に国債）\n\u2022 リターンは低め\n\nOpsにとって重要な理由：株式と債券は決済方法、コーポレートアクション、使用するシステムが異なります。Opsチームは資産クラス別に分かれていることが多いです（株式Ops、債券Opsなど）。',
      beginnerNote:'Stock = you become a co-owner. Bond = you become a lender. Both are securities that Ops processes every day.',
      beginnerNoteJa:'株式 = 共同オーナーになる。債券 = お金を貸す人になる。どちらもOpsが毎日処理する有価証券です。' },
    { id:'tq4', question:'What is operational risk? Can you give an example?', questionJa:'オペレーショナルリスクとは何ですか？例を挙げてください。',
      answer:'Operational risk is the risk of loss from failed or inadequate internal processes, people, systems, or external events.\n\nExamples:\n\u2022 Human error: An operator enters the wrong amount, settling $10M instead of $1M\n\u2022 System failure: A trading platform goes down during market hours\n\u2022 Process failure: A deadline is missed because the workflow was poorly designed\n\u2022 External event: A cyberattack disrupts settlement systems\n\nThis is different from market risk (prices moving against you) or credit risk (counterparty can\'t pay). Operational risk is about things going wrong in the process itself.\n\nWhy interviewers ask this: It shows you understand that Ops is fundamentally a risk management function. Every task in Ops, from reconciliation to settlement, exists to reduce operational risk.',
      answerJa:'オペレーショナルリスクとは、内部プロセス・人・システムの不備、または外部事象による損失リスクです。\n\n例：\n\u2022 人的ミス：担当者が金額を間違え、100万ドルではなく1,000万ドルで決済してしまう\n\u2022 システム障害：取引時間中にプラットフォームがダウンする\n\u2022 プロセスの不備：ワークフロー設計が悪く期限を逃す\n\u2022 外部事象：サイバー攻撃で決済システムが停止する\n\nこれは市場リスク（価格変動）や信用リスク（相手が払えない）とは異なります。オペレーショナルリスクはプロセス自体の問題です。\n\nなぜ聞かれるか：Opsが本質的にリスク管理の機能であることを理解しているか確認するためです。照合から決済まで、Opsの全業務はオペレーショナルリスクを減らすために存在します。',
      beginnerNote:'Think of it as "what could go wrong in the process?" A typo, a system crash, a missed deadline. Ops exists to prevent and fix these problems.',
      beginnerNoteJa:'「プロセスで何が間違いうるか？」と考えてみてください。入力ミス、システムダウン、期限切れ。Opsはこれらを防ぎ、修正するために存在します。' },
    { id:'tq5', question:'What does T+1 settlement mean?', questionJa:'T+1決済とは何ですか？',
      answer:'T+1 means a trade settles one business day after it is executed. T = Trade date, +1 = one business day later.\n\nThe US moved to T+1 for stocks and ETFs in May 2024 (previously T+2). Canada and Mexico moved at the same time.\n\nWhy it matters:\n\u2022 Less risk: The shorter the gap between trade and settlement, the less chance something goes wrong\n\u2022 More pressure on Ops: With only one day to catch and fix errors, everything needs to be fast and automated\n\u2022 Hot topic: Interviewers like this question because it is a recent, real change that directly affects Operations\n\nOther regions: India already moved to T+1 in 2023. The EU is considering it for around 2027. Japan and the UK are evaluating.',
      answerJa:'T+1とは、取引が約定の翌営業日に決済されること。T = 約定日、+1 = 1営業日後。\n\n米国は2024年5月に株式・ETFをT+1に移行（以前はT+2）。カナダとメキシコも同時に移行。\n\nなぜ重要か：\n\u2022 リスク低減：取引から決済までの期間が短いほど、問題が起きる可能性が減る\n\u2022 Opsへのプレッシャー増：エラーを見つけて修正する時間が1日しかなく、迅速さと自動化が必須\n\u2022 時事トピック：面接官が好む質問。最近の実際の変化で、Opsに直接影響するから\n\nその他の地域：インドは2023年にT+1導入済み。EUは2027年頃を検討中。日本と英国は評価中。',
      beginnerNote:'Settlement used to take 2 days, now it takes 1. Faster is safer, but it means Ops has less time to fix mistakes.',
      beginnerNoteJa:'以前は決済に2日かかっていましたが、今は1日。速い方が安全ですが、Opsがミスを直す時間が減ります。' },
    { id:'tq6', question:'What is reconciliation?', questionJa:'照合（リコンシリエーション）とは何ですか？',
      answer:'Reconciliation is comparing two sets of records to make sure they match.\n\nSimple example: You check your bank statement against your own spending records. If you spent $50 at a store but your bank shows $500, that is a "break" that needs investigation.\n\nIn Operations:\n\u2022 Trade reconciliation: Do our records match the counterparty\'s records?\n\u2022 Position reconciliation: Do our records of what we hold match the custodian\'s records?\n\u2022 Cash reconciliation: Does our cash balance match the bank\'s records?\n\nWhen records do not match (a "break"), Ops investigates the cause and fixes it. This is one of the most common daily tasks in Operations and catches errors before they become costly problems.',
      answerJa:'照合とは、2つの記録セットを比較して一致しているか確認することです。\n\n身近な例：自分の支出記録と銀行明細を照らし合わせる。お店で5,000円使ったのに銀行では50,000円と表示されていたら、それは調査が必要な「ブレーク」です。\n\nオペレーションでは：\n\u2022 取引照合：自社の記録とカウンターパーティの記録は一致しているか？\n\u2022 ポジション照合：保有記録とカストディアンの記録は一致しているか？\n\u2022 現金照合：現金残高と銀行の記録は一致しているか？\n\n記録が一致しない場合（「ブレーク」）、Opsが原因を調査し修正します。これはOpsの最も一般的な日次業務の一つで、コストのかかる問題になる前にエラーを検出します。',
      beginnerNote:'Like double-checking your bank statement against your receipts. If something does not match, you find out why.',
      beginnerNoteJa:'銀行の明細とレシートを照らし合わせるようなもの。合わなければ原因を調べます。' },
    { id:'tq7', question:'What is a corporate action? Give an example.', questionJa:'コーポレートアクションとは何ですか？例を挙げてください。',
      answer:'A corporate action is an event by a company that affects people who hold its stock or bonds.\n\nCommon examples:\n\u2022 Dividend: Company distributes profits to shareholders (e.g., $1 per share)\n\u2022 Stock split: A 2-for-1 split means you get 2 shares for every 1 you owned (price halves)\n\u2022 Merger: Two companies combine, your shares may convert to the new company\'s shares\n\nWhy Ops cares: When a company pays a dividend, Ops must make sure every shareholder gets the right amount on the right date. If Ops misses a dividend or processes a stock split incorrectly, the bank loses real money. Corporate actions have strict deadlines, so accuracy and timeliness are critical.',
      answerJa:'コーポレートアクションとは、企業が行う株式・債券の保有者に影響するイベントです。\n\n一般的な例：\n\u2022 配当：企業が利益を株主に分配する（例：1株あたり100円）\n\u2022 株式分割：2対1の分割なら、1株が2株になる（価格は半分に）\n\u2022 合併：2社が統合し、保有株が新会社の株に転換される場合がある\n\nOpsが重視する理由：企業が配当を支払う時、Opsは全株主に正しい金額を正しい日付で届ける必要があります。配当を見逃したり株式分割の処理を誤ると、銀行は実際にお金を失います。コーポレートアクションには厳格な期限があり、正確さと適時性が不可欠です。',
      beginnerNote:'When a company pays a dividend or splits its stock, someone needs to update all the records correctly. That someone is Ops.',
      beginnerNoteJa:'企業が配当を払ったり株式分割をする時、全記録を正しく更新する人が必要です。それがOpsです。' },
    { id:'tq8', question:'Why is accuracy important in Operations?', questionJa:'なぜオペレーションでは正確さが重要なのですか？',
      answer:'In Operations, even small mistakes can have big consequences:\n\n\u2022 Financial loss: Entering a wrong digit can mean settling millions more or less than intended\n\u2022 Regulatory penalties: Incorrect reporting can lead to fines from regulators\n\u2022 Reputation damage: Repeated errors make counterparties lose trust in your firm\n\u2022 Cascading failures: One error in settlement can cause multiple downstream failures\n\nThis is why Ops has so many checks and controls: reconciliation, four-eyes review (two people check the same thing), automated validation rules, and escalation procedures.\n\nInterviewers ask this to see if you understand the stakes and if you are someone who naturally pays attention to detail. The best answer connects accuracy to real business impact.',
      answerJa:'オペレーションでは、小さなミスが大きな結果につながります：\n\n\u2022 金銭的損失：桁を1つ間違えるだけで、何百万も多くまたは少なく決済してしまう\n\u2022 規制上のペナルティ：不正確な報告は規制当局からの罰金につながる\n\u2022 信用の毀損：ミスが繰り返されるとカウンターパーティの信頼を失う\n\u2022 連鎖的な失敗：決済の1つのエラーが複数の下流の失敗を引き起こす\n\nだからOpsには多くのチェックと管理があります：照合、ダブルチェック（2人で同じものを確認）、自動バリデーション、エスカレーション手順。\n\n面接官がこれを聞くのは、リスクの大きさを理解しているか、細部に注意を払える人かを見るためです。正確さをビジネスへの実際の影響と結びつけて答えるのがベストです。',
      beginnerNote:'A typo in a trade can cost millions. That is why Ops double-checks everything and has so many controls in place.',
      beginnerNoteJa:'取引の入力ミス1つで何百万もの損失になり得ます。だからOpsは全てをダブルチェックし、多くの管理体制を持っています。' },
];
...
function renderDashboard(){
    ...
    const sections = [
        ...
        {name:t('industryNews'), items:industryNewsData.reduce((a,c) => a.concat(c.articles), []), section:'industry-news'},
        {name:t('references'), items:referenceSources.reduce((a,c) => a.concat(c.links), []), section:'references'},
    ];
